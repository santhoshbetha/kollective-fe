...
import { useSettingsNotifications } from '../hooks/useSettingsNotifications.js';
...
const SidebarNavigationX = () => {
  ...
  const notificationCount = useBoundStore((state) => state.notifications.unread);
  const followRequestsCount = useBoundStore((state) => state.user_lists.follow_requests.items.count());
  const dashboardCount = useBoundStore((state) => state.admin.openReports.count() + state.admin.awaitingApproval.count());
