// src/features/posts/components/LikeButton.jsx
// Quick Reactions
...
import { useToggleFavourite } from "../api/usePostActions";

const LikeButton = ({ post }) => {
  const { show, hide, activePostId, isVisible } = useQuickReactionStore();
  ...
  const handleClick = () => {
    if (!isVisible) {
      toggleLike({ id: post.id, isLiked: post.favourited });
    }
  };
  ...
  return (
    <div className="relative">
      <button
        ...
        onClick={handleClick}
        className={post.favourited ? 'text-red-500' : ''}
      >
        {post.favourited ? <HeartFull /> : <HeartEmpty />}
      </button>

      {/* Floating Bar: Only visible on long-press */}
      {isVisible && activePostId === post.id && (
        <QuickReactionLoader onSelect={hide} post={post} />
      )}
    </div>
  );
};
...
const QuickReactionLoader = ({ post, onSelect }) => {
  const { mutate: react } = useExclusiveReaction(post);
