...
import { fetchContext } from '../api/posts';
import { buildAncestors, buildDescendants } from '../utils/traversal';
import { postKeys } from '@/queries/keys';

export const useConversation = (conversationId) => {
  const query = useQuery({
    // Renamed parameter to conversationId for clarity with your usePost hook
    queryKey: postKeys.context(conversationId), //['post', 'context', conversationId],
...
/*
//src/features/posts/components/ConversationTree.tsx:
import { useConversation } from '../hooks/use-conversation';

export const ConversationTree = ({ postId }: { postId: string }) => {
  // Call the hook
  const { data, ancestors, descendants, isLoading, isError, error } = useConversation(postId);
  ...
  return (
    <div className="thread-container">
      ...
      <div className="ancestors">
        {ancestors?.map(id => <PostItem key={id} id={id} />)}
      </div>

      {/* Render Main Post *//*}
      <div className="current-post">
        <PostItem id={postId} isHighlighted />
      </div>

      {/* Render Descendants */ /*}
      <div className="descendants">
        {descendants?.map(id => <PostItem key={id} id={id} />)}
      </div>
    </div>
  );
};
