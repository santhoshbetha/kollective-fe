...
export const fetchPostContext = async (postId) => {
  // 1. Fetch the main post and the thread context in parallel
  const [postRes, contextRes] = await Promise.all([
    fetch(`/api/v1/posts/${postId}`),
    fetch(`/api/v1/posts/${postId}/context`)
  ]);

  if (!postRes.ok || !contextRes.ok) {
  ...
  // 2. Create a Map of EVERY post in the thread for O(1) lookup
  ...
  context.ancestors.forEach(item => allPosts.set(item.id, item));

  // Add all replies (descendants)
  context.descendants.forEach(item => allPosts.set(item.id, item));
  ...
  return {
    ...
    allPosts, // This is what your usePost hook uses in initialData!
  };
};

export const fetchConversation = async (postId) => {
  ...
  [...data.ancestors, ...data.descendants].forEach((post) => {
    if (post.in_reply_to_id) {
      inReplyTos.set(post.id, post.in_reply_to_id);
      const existing = replies.get(post.in_reply_to_id) || [];
      replies.set(post.in_reply_to_id, [...existing, post.id]);
    }
  });
...
Why this is a "Best Practice"
    ...
    React Router triggers a re-render, and your usePost hooks automatically update because the conversationId dependency changed.
...
/*
postId: This is the unique ID of one specific message. Every post on the platform has its own postId.
...
Analogy: If a post is a "Page," the conversation is the "Book." If you have the Book ID, 
you can find all the Pages. 
*/

/*
The Context API: .../posts/${postId}/context
...
The API returns a Context object containing two arrays: 
    ...
    ancestors: An array of postsposted before this one in the same thread (the parents,
     grandparents, etc.).
    descendants: An array of postsposted after this one (the replies). 

Crucial Note: The Context API typically does not include the post itself—only the messages 
...
Here is how you would use the data from fetchPostContext to build a vertical timeline.

export const ConversationThread = ({ postId }) => {
  // 1. Fetch the processed context (the "Book")
  const { data: thread, isLoading } = useConversation(postId);
  ...
  const fullThread = [
    ...thread.ancestors,
    thread.mainPost, // This is the post the user clicked
    ...thread.descendants
  ];
...
Visual Structure of the List
    ...
    Main Post: Often styled with a border or background (via isHighlighted) to show it is the "anchor" of the page.
    Descendants: These appear at the bottom as replies.

See PostItem2.jsx for the component that renders each individual post with a vertical line connecting them.

*/

/* FULL THREAD:
To build the
fullThread list properly, you need to combine the ancestors, the mainPost, and the descendants into a single array.
...
const fullThread = [
  ...
  thread.mainPost,    // The post the user is actually viewing
...
export const ConversationThread = ({ postId }: { postId: string }) => {
  // 1. Fetch the data (ancestors, mainPost, descendants)
  const { data: thread, isLoading, isError } = useConversation(postId);
    ...
    <div className="conversation-thread">
      {fullThread.map((post) => (
        <PostItem
          key={post.id}
          id={post.id}
          conversationId={postId} // Pass the anchor ID as the context
          isHighlighted={post.id === postId} // Highlight the main post
        />
      ))}
    </div>
  );
...
/* THREAD GAP:
Sometimes a thread has a missing link (e.g., a deleted post or a post from a server your instance hasn't seen). In your fullThread.map, you can check if post.in_reply_to_id matches the
...
To detect this, you compare each post's in_reply_to_id with the id of the post directly preceding it in your fullThread array. 
...
{fullThread.map((post, index) => {
  const previousPost = fullThread[index - 1];
  
  // Check if this post is a reply to the one above it
  // We ignore index 0 because it's the start of our list
  const isGap = index > 0 && post.in_reply_to_id !== previouspost.id;
    ...
    <React.Fragment key={post.id}>
      {isGap && (
        <div className="thread-gap">
          ...
          <button onClick={() => handleFetchGap(post.in_reply_to_id)}>
            Load Context
          </button>
        </div>
      )}
...
4. Resolving the Gap:
When a user clicks "Load Context," you can trigger a new fetchPostContext for that 
specific post.in_reply_to_id. This effectively "backfills" the missing branch of the conversation. 
*/
...
In TanStack Query, this is done using queryClient.setQueryData to manually update the PostContext Map so the UI refreshes instantly without reloading the entire page.

import { useQueryClient } from '@tanstack/react-query';
import { fetchPostContext } from '../api/posts';

export const useThreadActions = (conversationId) => {
  const queryClient = useQueryClient();

  const handleFetchGap = async (missingPostId) => {
    try {
      // 1. Fetch the context for the "missing" parent post
      // This returns { ancestors, descendants, allPosts (Map) }
      const newContext = await fetchPostContext(missingPostId);

      // 2. Update the existing conversation cache
      queryClient.setQueryData(['post', 'context', conversationId], (oldData) => {
        if (!oldData) return oldData;

        // Create a new Map to ensure React detects the change
        const updatedMap = new Map(oldData.allPosts);

        // Merge all newly found postsinto the master Map
        newContext.allPosts.forEach((post, id) => {
          updatedMap.set(id, post);
        });
...
Why this works
    ...
    Cache Merging: By adding the new posts to the allPosts Map, any PostItem that was previously 
...
1. Update the Hook
Add a simple state to track which missingPostId is currently being fetched.
...
export const useThreadActions = (conversationId: string) => {
  ...
  const handleFetchGap = async (missingPostId: string) => {
    setLoadingGapId(missingPostId); // Start loading
    try {
      const newContext = await fetchPostContext(missingPostId);
      
      queryClient.setQueryData(['post', 'context', conversationId], (oldData: any) => {
        if (!oldData) return oldData;
        const updatedMap = new Map(oldData.allPosts);
        newContext.allPosts.forEach((post: any, id: string) => {
          updatedMap.set(id, post);
        });
        return { ...oldData, allPosts: updatedMap };
      });
    } catch (error) {
      console.error("Could not bridge thread gap:", error);
    } finally {
...
{isGap && (
  <div className="thread-gap">
    <button 
      onClick={() => handleFetchGap(post.in_reply_to_id)}
      disabled={loadingGapId === post.in_reply_to_id} // Disable while fetching
      className="gap-button"
    >
      {loadingGapId === post.in_reply_to_id ? (
        <>
    ...
    <React.Fragment key={post.id}>
      {isGap && (
        <AutomaticThreadGap 
          ...
          isLoading={loadingGapId === post.in_reply_to_id}
        />
      )}
      <PostItem id={post.id} conversationId={conversationId} />
    </React.Fragment>
  );
})}
...
1. The Batched Hook
This hook collects multiple missingPostId calls into a single queue and processes them together.
  ...
  const processBatch = useCallback(async () => {
    ...
    try {
      ...
      const results = await Promise.all(
        idsToFetch.map(id => fetchPostContext(id))
      );
      ...
      queryClient.setQueryData(['post', 'context', conversationId], (oldData: any) => {
        ...
        results.forEach(context => {
          context.allPosts.forEach((post: any, id: string) => {
            updatedMap.set(id, post);
          });
        });
...
2. Benefits of Batching
    ...
    Minimized UI Re-renders: Instead of updating the React Query cache five times (triggering five re-renders), the code updates the cache once with all newly found posts.
...
First, update your fetchPostContext or create a wrapper that includes a retry counter. We use Exponential Backoff, which doubles the wait time after each failure (e.g., 1s, then 2s, then 4s) to avoid overwhelming the server.
...
const processBatch = useCallback(async () => {
  ...
  try {
    ...
    const results = await fetchWithRetry(() => 
      Promise.all(idsToFetch.map(id => fetchPostContext(id))),
      3, // Max 3 retries
...
export const ConversationThread = ({ postId }: { postId: string }) => {
  ...
  const fullThread = [...thread.ancestors, thread.mainPost, ...thread.descendants];
  ...
  return (
    <div className="relative">
      ...
      {fullThread.map((post) => (
        <div 
          key={post.id} 
          // Attach the ref ONLY if this is the target postId
          ref={post.id === postId ? anchorRef : null}
        >
          <PostItem 
            id={post.id} 
            conversationId={postId} 
...
useEffect(() => {
  if (thread) {
    const anchorIndex = fullThread.findIndex(s => s.id === postId);
...
Pass the "active" state to each PostItem and use a Dynamic Ref to handle the scrolling.
...
Inside your PostItem, use a useEffect to scroll the post into view whenever it becomes active.
...
In your ConversationThread component, track which post is currently being replied to.
...
const handleOpenReply = () => {
  const currentPost = fullThread[selectedIndex];
  if (currentPost) {
    setReplyingToPost(currentPost);
  }
};
...
When replyingToPost is not null, show your modal. Ensure the textarea inside the modal has an autoFocus attribute so the user can start typing immediately.
...
const ReplyModal = ({ post, onClose }) => {
  ...
  const submitReply = () => {
    sendReply({ postId: post.id, content: text }, {
      onSuccess: () => {
  ...
  return (
    <div className="modal-backdrop">
      <div className="modal-content">
        <h3>Replying to {post.account.display_name}</h3>
        <textarea
...
When the modal closes, focus should return to the original PostItem so the user can 
...
export const useSendReply = (conversationId: string) => {
  ...
  return useMutation({
    ...
    onMutate: async (newReply) => {
      ...
      // 3. Create an "Optimistic" post object
      ...
      queryClient.setQueryData(['post', 'context', conversationId], (old: any) => {
        const updatedMap = new Map(old.allPosts);
        updatedMap.set(tempId, optimisticPost);
        
        return {
          ...old,
          allPosts: updatedMap,
          // Add to descendants list so it shows up at the bottom
          descendants: [...old.descendants, optimisticPost]
        };
      });
...
Inside your PostItem, you can use the isOptimistic flag we added to show the post is still "syncing."


/* src/styles/post.css
//.post-item.optimistic {
//  opacity: 0.6;
//  filter: grayscale(50%);
...
Note: To prevent duplication, you must ensure your allPosts Map logic is stable during the TanStack Query cache update.
...
export const useDeletePost = (postId, conversationId) => {
  const queryClient = useQueryClient();
...
// Inside your useDeletePost hook
onMutate: async (postId) => {
  ...
  queryClient.setQueryData(['post', 'context', conversationId], (old: any) => {
    const updatedMap = new Map(old.allPosts);
    const item = updatedMap.get(postId);
    if (item) {
      updatedMap.set(postId, { ...item, isDeleting: true });
    }
    return { ...old, allPosts: updatedMap };
  });
...
export const PostItem = ({ post, ...props }) => {
  ...
  const PostClasses = `post-item ${post.isDeleting ? 'deleting' : ''}`;
...
/*
implement "Edit Mode" where the post content turns into an input field for quick corrections
...
In your PostItem component, track whether the user is currently editing and store the "draft" text.

export const PostItem = ({ post, conversationId }) => {
  const [isEditing, setIsEditing] = useState(false);
  const [draftContent, setDraftContent] = useState(post.content);
  const { mutate: updatePost, isPending } = useUpdatePost(post.id, conversationId);

  const handleSave = () => {
    updatePost({ content: draftContent }, {
      onSuccess: () => setIsEditing(false)
    });
  };

  return (
    <article className={`post-item ${isEditing ? 'editing' : ''}`}>
      {isEditing ? (
...
edited_at field returned by the Mastodon Post Entity. This field is only populated if the post has been edited at least once. 
    ...
    <article className="post-item">
      <header>
        ...
        <div className="meta-info">
          ...
          {post.edited_at && (
            <span className="edited-badge" title={`Original: ${post.created_at}`}>
              • Edited <TimeAgo date={post.edited_at} />
            </span>
          )}
        </div>
      </header>
...
If you want users to see what changed, you can use the Mastodon API's /api/v1/posts/{id}/history endpoint. This returns an array of PostEdit objects showing previous versions of the content. 

    Logic: Only show an "Edit History" button if post.edited_at is present.
    Data: The history includes the content and created_at for every revision. 


These technical references cover the Mastodon "PostEdit" entity and the API endpoint for fetching post edit history

1)https://docs.joinmastodon.org/entities/PostEdit/
...
To implement a
History Modal, you use the Mastodon API's GET /api/v1/posts/:id/history endpoint. This returns an array of PostEdit objects, allowing users to see every version of a post from its initial state to the current one. 
1. The API Fetcher
Create a function in your api/posts.js to retrieve the version history. 

export const fetchPostHistory = async (postId) => {
  const response = await fetch(`/api/v1/posts/${postId}/history`);
  if (!response.ok) throw new Error("Failed to fetch edit history");
  return response.json(); // Returns an array of PostEdit objects
};

2. The custom hook: usePostHistory
...
export const PostHistoryModal = ({ postId, onClose }) => {
  const { data: history, isLoading } = usePostHistory(postId);
...
To implement a
"Restore Version" feature, you effectively take the content from a previous version in the history list and trigger your useUpdatePost mutation to overwrite the current post.
...
export const PostHistoryModal = ({ post, onClose }) => {
  const { data: history } = usePostHistory(post.id);
  // Re-use our existing update mutation
  const { mutate: updatePost, isPending } = useUpdatePost(post.id, post.conversationId);

  const handleRestore = (historicalContent: string) => {
    if (window.confirm("Are you sure you want to restore this version?")) {
      updatePost({ content: historicalContent }, {
        onSuccess: () => {
...
2. How it works
    ...
    Action: Clicking "Restore" passes the revision.content string into your updatePost function.
    Cache Sync: Because useUpdatePost (which we built earlier) already handles updating the TanStack Query cache, the main thread UI will update instantly to show the restored content.

3. Safety Check
Since the Mastodon API returns HTML for post content, the revision.content is already formatted correctly. However, your backend must support Post Editing (available since Mastodon v3.5.0) for the PATCH request to work.
...
To implement a
Compare Mode, you can use a diffing library to highlight exactly what changed between two versions. Since Mastodon postsare stored as HTML, you have two choices: a Side-by-Side Text Diff (standard for code) or an Inline HTML Diff (visual, like "Track Changes").
...
In the Mastodon API, post history includes a media_attachments array for each version. To compare them, you check the differences between the current version's media and the historical version's media.
...
/*
 ...
 To implement
Poll History, you track how a poll’s options, type, or duration have changed across different versions of a post. In the Mastodon API, a PostEdit entity includes a poll field representing the state of the poll at that specific revision. 
...
In your history modal, display a "Poll Comparison" section.
    ...
    Post Indicators: If optionsChanged is true, add a notice: "⚠️ This edit reset the poll results".
...
These resources detail the Mastodon API's PostEdit entity and the "poll" field, explaining how to identify and visualize changes in poll options, type, and expiration
...
Since full multi-user typing (like Google Docs) is complex, most apps use a Locking Pattern. When one user starts editing, the post is "locked" for others. 

    Acquire Lock: When a user clicks "Edit," send a JOIN_EDIT event via WebSocket to notify the server.
    Broadcast Presence: The server tells all other connected clients: "User X is currently editing Post #123".
...
Pusher Presence Channels are ideal for this because they automatically track who is "online" in a specific room (post ID).

// Join the 'presence-post-123' channel when entering Edit Mode
const channel = pusher.subscribe(`presence-post-${postId}`);
...
The most common approach is to use ETags or Version Numbers. 
    ...
    The Check: When you fetch a post, the server sends a unique identifier (like v1 or an ETag).
...
const handleSaveMedia = (currentMediaId, updatedData) => {
  // 1. Fetch the latest post from the server
  const latestPost = await fetchPost(postId);
  
  // 2. Verify the ID still exists in the latest version
  const mediaExists = latestpost.media_attachments.some(m => m.id === currentMediaId);
...
You can use the onMutate logic to check for these mismatches optimistically.
    ...
    Logic: If the postId being updated has a different media_attachment count or different IDs than what is in your local state, trigger a "Merge View" for the media.
...
4. Integration Details
    ...
    Mastodon Specifics: In Mastodon, you can upload up to four images per post. Larger media like video or GIFV are processed asynchronously and may return a 202 Accepted post while processing continues in the background.
...
const uploadBatch = async (files) => {
  ...
  const failedUploads = results.filter(r => r.post === 'rejected');
...
To implement a
Manual Retry button, we need to track each file's post (e.g., idle, uploading, success, or error) in our state. This allows the UI to show a specific "Retry" icon only on the thumbnail that failed.
...
const uploadSingleFile = async (fileName: string) => {
  ...
  setFiles(prev => ({
    ...prev,
    [fileName]: { ...prev[fileName], post: 'uploading', progress: 0 }
  }));
  ...
  try {
    ...
    setFiles(prev => ({
      ...prev,
      [fileName]: { ...prev[fileName], post: 'success', progress: 100 }
    }));
    ...
    setFiles(prev => ({
      ...prev,
      [fileName]: { ...prev[fileName], post: 'error', errorMsg: "Upload failed" }
    }));
  }
};

3. The Thumbnail UI with Retry Button
Use an overlay on the image thumbnail. If the post is error, show a "refresh" icon.
...
4. Why this UX works
    ...
    Disabled Form: You can disable the main "Post" button if any file in the list has the error or uploading post.
