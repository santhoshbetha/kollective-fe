// src/features/posts/api/useTimeline.ts
...
export const useHomeTimeline = () => {
  ...
  return useInfiniteQuery({
    ...
    // We take the ID of the last post in the current list
    ...
    select: (data) => ({
        pages: data.pages.map(page => page.filter(post => !post.muted)),
...
export const useHomeTimeline2 = () => {
  return useInfiniteQuery({
    ...
    queryFn: async ({ pageParam }) => {
      ...
      // Validate and Enrich every post in the array
      return data.map(rawPost => PostSchema.parse(rawPost));
    },
...
/*
Your components become much simpler because they don't need to call getReactForPost or reduceEmoji anymore.

const PostActions = ({ post }) => {
  // Data is already enriched by the schema!
  const myReaction = post.myReaction;
  const allReactions = post.enrichedReactions;
...
Why this is the "Ultimate" Migration Step:
    ...
    1. Runtime Safety: If the server sends an invalid ID or a null count, PostSchema.parse will 
...
Why this is a "Production-Grade" Migration:
    ...
    3. Consistency: Your Zod schemas now act as the "Single Source of Truth" for your entire data flow—from Auth to posts to Reactions.
...
// src/features/posts/api/useTimeline.js

export const useTimeline2 = (type = 'home') => {
  return useInfiniteQuery({
    queryKey: [posts', 'timeline', type],
    queryFn: fetchTimeline,
...
3. Why this is critical for kollective:-FE
    ...
    DOM Stability: By pruning the data, you can ensure your virtualized list (or standard list) doesn't have to manage 5,000 post items at once.
    Automatic Management: In your postsSlice.js, you would have needed complex if (items.length > 500) logic. Here, it’s a single line of configuration.
*/
    ...
    select: (data) => {
      ...
      return {
        ...
        pages: data.pages.map(page => ({
          ...page,
          // Remove poststhat contain muted keywords
          items: page.items.filter(post => {
            const content = (post.content + post.spoiler_text).toLowerCase();
    ...
    select: (data) => ({
      ...
      pages: data.pages.map(page => ({
        ...
        items: page.items.map(post => ({
          ...
          shouldShowContent: prefs?.['reading:expand:spoilers'] || !post.spoiler_text
        }))
      }))
    })
  });
...
const PostCard = ({ post }) => {
  ...
  const { data: rel } = useRelationship(post.account.id);
  
  // Local state for manual toggle
  const [manualExpand, setManualExpand] = useState(false);

  // LOGIC: Show content if manually toggled OR if the user is 'pinned' (trusted)
  const isExpanded = post.shouldShowContent || manualExpand || rel?.pinned;

  if (post.spoiler_text && !isExpanded) {
    return (
      <div className="cw-wrapper">
        <span>Content Warning: {post.spoiler_text}</span>
    ...
    select: (data) => ({
      ...
      pages: data.pages.map(page => ({
        ...
        items: page.items.map(post => ({
          ...
          isMuted: post.account.muted || false 
        }))
      }))
    })
  });
...
const PostCard = ({ post }) => {
  ...
  if (post.isMuted && !revealMuted) {
    return (
      <div className="muted-placeholder">
        <span>Post by {post.account.acct} (Muted)</span>
...
export const useTimeline6 = (type = 'home') => {
  return useInfiniteQuery({
    ...
    queryFn: async ({ pageParam }) => {
      ...
      // VALIDATION: Parse the array of posts
...
// Generic Timeline Hook. You can delete the timelinesSlice, pending_posts, 
...
export const useTimeline7 = (timelineId, endpoint, params = {}) => {
  ...
  return useInfiniteQuery({
    ...
    queryKey: [posts', 'timeline', timelineId, params],
    queryFn: async ({ pageParam }) => {
      ...
      // SIDE-LOADING: Seed the global post/account cache
      // Replaces importFetchedPosts(data)
    ...
    // Logic Port: "Pending posts" / Dequeueing logic
...
expandAccountTimeline	useTimeline('account', '/api/v1/accounts/' + id + '/posts')
  ...
  return useInfiniteQuery({
    ...
    select: (data) => {
      ...
      return {
        ...
        pages: data.pages.map(page => ({
          ...page,
          // Nuke the post from the array entirely if it's irreversible
          items: page.items.filter(post => !isIrreversible(post, activeFilters))
        }))
      };
    }
  });
